...
const INPUT_URL = ""; // location of input files
...
const INPUT_LIGHT_URL = INPUT_URL + "lights.json";

var defaultEye = vec3.fromValues(0.5,0.3,-0.1);
...
var lightPosition = vec3.fromValues(0.5,2,-0.3);
var rotateTheta = Math.PI/100;
...
function getBinaryFile(url) {
    try {
            ...
            if ((httpReq.status !== 200) || (httpReq.readyState !== XMLHttpRequest.DONE))
                ...
                return stringToArrayBuffer(httpReq.response);
        }
    catch(e) {
        console.log(e);
        return(null);
    ...
    try {
      ...
      else
      {
        ...
        gl.enable(gl.DEPTH_TEST); // use hidden surface removal (with zbuffering)
          var gext = gl.getExtension('OES_texture_float');
          gext = gl.getExtension('OES_texture_float_linear') ;
          gext = gl.getExtension('OES_texture_half_float') ;
          gext = gl.getExtension('OES_texture_half_float_linear');
      }
    } // end try
...
function loadModels() {
    ...
    function loadTexture(whichModel,currModel,textureFile) {
        ...
        if (textureFile != false)
        {
            if(1 == ishdr(textureFile))
            {
                var vfile = getBinaryFile(textureFile);
                {
                    ...
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA,vdata.shape[0],vdata.shape[1],0, gl.RGBA, gl.FLOAT,vdata.data);
                    setsingletexture(vdata.shape.width,vdata.shape.height);
            ...
            else
            {
                ...
                { // when texture image loaded...
                    ...
                    setsingletexture(currTexture.image.width,currTexture.image.height);
                    gl.bindTexture(gl.TEXTURE_2D, null); // deactivate model's new texture
...
{
    var vobjtype = "lgihts";
    var vstr = getJSONFile(INPUT_LIGHT_URL,vobjtype);
    if(String.null === vstr)
    {
...
function setlights()
{
    for(var v = 0; v < g_lights.length; v++)
    {
        var lightAmbientULoc = gl.getUniformLocation(shaderProgram, "uLightAmbient[" + v+"]"); // ptr to light ambient
        var lightDiffuseULoc = gl.getUniformLocation(shaderProgram, "uLightDiffuse[" + v+"]"); // ptr to light diffuse
        var lightSpecularULoc = gl.getUniformLocation(shaderProgram, "uLightSpecular[" + v+"]"); // ptr to light specular
        var lightPositionULoc = gl.getUniformLocation(shaderProgram, "uLightPosition[" + v+"]"); // ptr to light position

        gl.uniform3fv(lightAmbientULoc,g_lights[v].amb_color); // pass in the light's ambient emission
        gl.uniform3fv(lightDiffuseULoc,g_lights[v].dif_color); // pass in the light's diffuse emission
        gl.uniform3fv(lightSpecularULoc,g_lights[v].spc_color); // pass in the light's specular emission
        gl.uniform3fv(lightPositionULoc,g_lights[v].pos); // pass in the light's position
    }
    var lightnum = gl.getUniformLocation(shaderProgram, "lightnumber");
    gl.uniform1i(lightnum,g_lights.length);
}
...
function updatetonemode()
{
    //tonemappingmode
    var tonemode = gl.getUniformLocation(shaderProgram, "tonemappingmode");
    gl.uniform1i(tonemode,g_tone_mode);
}

function updatetoneparam(val)
{
    //tonemappingmode
    var expo = gl.getUniformLocation(shaderProgram, "exposure");
    gl.uniform1f(expo,val);
}



function rotateobj(axis,direction,idx)
{
    var newRotation = mat4.create();
    mat4.fromRotation(newRotation,direction*rotateTheta,axis); // get a rotation matrix around passed axis
    vec3.transformMat4(inputSpheres[idx].xAxis,inputSpheres[idx].xAxis,newRotation); // rotate model x axis tip
    vec3.transformMat4(inputSpheres[idx].yAxis,inputSpheres[idx].yAxis,newRotation); // rotate model y axis tip
}
