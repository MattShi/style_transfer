...
function getJSONFileByCode(url,descr)
{
    if(descr === "triangles")
    {
        ...
        var str = "[{\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,1,0],\"vertices\": [[0,0,0],[1,0,0],[1,0,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,1,0],\"vertices\": [[1,0,1],[0,0,1],[0,0,0]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[-1,0,0],\"vertices\": [[1,0,0],[1,0,1],[1,1,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[-1,0,0],\"vertices\": [[1,1,1],[1,1,0],[1,0,0]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.6,0.0,0.0], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,0,-1],\"vertices\": [[0,0,1],[1,0,1],[1,1,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.6,0.0,0.0], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,0,-1],\"vertices\": [[1,1,1],[0,1,1],[0,0,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[1,0,0],\"vertices\": [[0,0,0],[0,0,1],[0,1,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[1,0,0],\"vertices\": [[0,1,1],[0,1,0],[0,0,0]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,-1,0],\"vertices\": [[0,1,0],[1,1,0],[1,1,1]]},\
                    {\"ambient\": [0.1,0.1,0.1], \"diffuse\": [0.0,0.0,0.6], \"specular\": [0.3,0.3,0.3], \"n\":1,\"normal\":[0,-1,0],\"vertices\": [[1,1,1],[0,1,1],[0,1,0]]}\
        ]";
...
function shadeIsectSphere(isect, isectSphere, lights, spheres,triangles,viewpos)
{
    try
    {
        var c = new Color(0,0,0,255);
        var sphere = spheres[isectSphere];
        var lightOccluded = false;
        ...
        for (var lightidx=0; lightidx<lights.length; lightidx++)
        {
            ...
            var L = Vector.normalize(Vector.subtract(lpos,isect.xyz)); // light vector unnorm'd
            var N = Vector.normalize(Vector.subtract(isect.xyz,sphereCenter)); // surface normal
            var cosnl = Vector.dot(N,L)/(Vector.length(N)*Vector.length(L));
            if(cosnl < 0)
            {
                cosnl = 0.0;
            }
            ...
            {
                c[0] += lights[lightidx].diffuse[0] * sphere.diffuse[0] ;
                c[1] += lights[lightidx].diffuse[1] * sphere.diffuse[1] ;
                c[2] += lights[lightidx].diffuse[2] * sphere.diffuse[2] ;

                var V = Vector.normalize(Vector.subtract(viewpos,isect.xyz));
                var H = Vector.normalize(Vector.add(L,V));
                var specFactor = Math.max(0,Vector.dot(N,H));
                if (specFactor > 0)
                {
                    var newSpecFactor = specFactor;
                    for (var s=1; s<spheres[isectSphere].n; s++)
                    {
...
function shadeIsectTriangle(isect, isecTriangle, lights,spheres,triangles,viewpos)
{
    try {
        var c = new Color(0,0,0,255);
        var triangle = triangles[isecTriangle];
        var lightOccluded = false;
        for (var lightidx=0; lightidx<lights.length; lightidx++)
        {
            var lpos = new Vector(lights[lightidx].x, lights[lightidx].y, lights[lightidx].z);
            ...
            if (!isLightOccluded(lpos, isect.xyz, spheres, triangles, -1, isecTriangle))
            {
                ...
                c[0] += lights[lightidx].diffuse[0] * triangle.diffuse[0] ;
                c[1] += lights[lightidx].diffuse[1] * triangle.diffuse[1] ;
                c[2] += lights[lightidx].diffuse[2] * triangle.diffuse[2] ;

                var V = Vector.normalize(Vector.subtract(viewpos, isect.xyz)); // view vector
...
function drawbyintersection(inputSpheres,inputLights,inputTriangles,vnearstisec,viewpos)
{
    ...
    if((vnearstisec.exists)&& (vnearstisec.id >= 0))
    {
        if(vnearstisec.type == 0)
        {
            ...
            c = shadeIsectSphere(vnearstisec, vnearstisec.id, inputLights, inputSpheres,inputTriangles,viewpos);
        }
        else
        {
            ...
            c = shadeIsectTriangle(vnearstisec, vnearstisec.id, inputLights,inputSpheres, inputTriangles,viewpos);
        }
    }
...
//
function getlightingparam(inputSpheres,inputtirangles,type,id)
{
    if(type == 0) //sphere
    {
        return {"a":inputSpheres[id].ambient,"d":inputSpheres[id].diffuse,"s":inputSpheres[id].specular};
    }
    else
    {
        return {"a":inputtirangles[id].ambient,"d":inputtirangles[id].diffuse,"s":inputtirangles[id].specular}
    }
}

function calselfemit(inputSpheres, inputLights, inputTriangles, type,id)
{
    var vc = new Color(0,0,0,255);
    var vm = getlightingparam(inputSpheres, inputTriangles, type,id);
    for(var vl = 0; vl < inputLights.length;vl++)
    {
        vc[0] += inputLights[vl].ambient[0]*vm.a[0];
        vc[1] += inputLights[vl].ambient[0]*vm.a[1];
        vc[2] += inputLights[vl].ambient[0]*vm.a[2];
    }
    return vc;
}
...
//amb/*no need */ + diff + spc
function caldirectr(inputSpheres, inputLights, inputTriangles, isect,hitid, viewpos)
{
    var c = new Color(0,0,0,255);
    try
    {
      if(hittype == 0 )
      {
          var sphere = inputSpheres[hitid];
          var lightOccluded = false;

          for (var lightidx=0; lightidx<inputLights.length; lightidx++) {
              var lpos = new Vector(inputLights[lightidx].x, inputLights[lightidx].y, inputLights[lightidx].z);
              var L = Vector.subtract(lpos, isect.xyz);

              if (!isLightOccluded(lpos, isect.xyz, inputSpheres, inputTriangles, hitid, -1))
              {
                  c[0] += inputLights[lightidx].diffuse[0] * sphere.diffuse[0];
                  c[1] += inputLights[lightidx].diffuse[1] * sphere.diffuse[1];
                  c[2] += inputLights[lightidx].diffuse[2] * sphere.diffuse[2];
                  ...
                  if (specFactor > 0) {
                      ...
                      c[0] += inputLights[lightidx].specular[0] * sphere.specular[0] * newSpecFactor; // specular term
                      c[1] += inputLights[lightidx].specular[1] * sphere.specular[1] * newSpecFactor; // specular term
                      c[2] += inputLights[lightidx].specular[2] * sphere.specular[2] * newSpecFactor; // specular term
                  }
              }
          }
      }
      ...
      {
          var triangle = inputTriangles[hitid];
          var lightOccluded = false;
          for (var lightidx=0; lightidx<inputLights.length; lightidx++)
          {
              var lpos = new Vector(inputLights[lightidx].x,inputLights[lightidx].y,inputLights[lightidx].z);
              lpos.set(inputLights[lightidx].x,inputLights[lightidx].y,inputLights[lightidx].z);

              var L = Vector.subtract(lpos,isect.xyz);
              if (!isLightOccluded(lpos,isect.xyz,inputSpheres,inputTriangles,-1,hitid))
              {
                  var N = Vector.normalize(new Vector(triangle.normal[0],triangle.normal[1],triangle.normal[2]));

                  c[0] += inputLights[lightidx].diffuse[0] * triangle.diffuse[0] ;
                  c[1] += inputLights[lightidx].diffuse[1] * triangle.diffuse[1] ;
                  c[2] += inputLights[lightidx].diffuse[2] * triangle.diffuse[2] ;
                  ...
                  if (specFactor > 0)
                  {
                      ...
                      c[0] += inputLights[lightidx].specular[0] * triangle.specular[0] * newSpecFactor; // specular term
                      c[1] += inputLights[lightidx].specular[1] * triangle.specular[1] * newSpecFactor; // specular term
                      c[2] += inputLights[lightidx].specular[2] * triangle.specular[2] * newSpecFactor; // specular term
                  }
              }
          }
      }
...
function raytracingforall(inputSpheres, inputLights, inputTriangles, pos, dir, times)
{
    ...
    {
        var cemitcolor = calselfemit(inputSpheres,inputLights,inputTriangles,vnearstisec.type,vnearstisec.id);
        var drawrt = drawbyintersection(inputSpheres,inputLights,inputTriangles,vnearstisec,pos);  //within cosnl
        var cdirectcolor = drawrt.color;

        c[0] += cdirectcolor[0] + cemitcolor[0];
        c[1] += cdirectcolor[1] + cemitcolor[1];
        c[2] += cdirectcolor[2] + cemitcolor[2];

        if(russianRoulette() && (G_USE_RAYTRACING_MORE > 0) && (G_USE_BRDF > 0) )
        {
            var vnextdir = getrandomdir(drawrt.n);
            var vf = calBRDFColor(drawrt.n,vnextdir,Vector.scale(-1,dir));
            if(vf < 0)
            {
                vf = 0;
            }
            var vnextcolor = raytracingforall(inputSpheres, inputLights, inputTriangles, vnearstisec.xyz, vnextdir, times);
            c[0] += vnextcolor[0];
            c[1] += vnextcolor[1];
            c[2] += vnextcolor[2];
        }
...
function drawScene(inputSpheres,inputLights,inputTriangles,pos,dir,times)
{
    ...
    c[0] = Math.min(255,c[0]*255);
    c[1] = Math.min(255,c[1]*255);
    c[2] = Math.min(255,c[2]*255);
    return c;
...
function raytracing(inputSpheres,inputLights,inputTriangles,pos,dir)
{
    ...
    var vnearstisec = {"exists": false, "xyz": NaN, "t": NaN,"type":NaN};
